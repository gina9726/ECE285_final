...
import pdb
import os
from vqaDataset_mask import vqaDataset
from vqaModel_mask import vqaModel
...
def train(model_path='', param_only=False):
    ...
    for ep in range(start_ep, epoch):
        for step, value in enumerate(train_loader):
            ...
            batch_ans_mask = Variable(value[2].type(dtype), requires_grad=False).cuda()
            batch_ans = Variable(value[3].type(torch.LongTensor), requires_grad=False).cuda()

            output, ans_mask = vqa(batch_imfeat, batch_ques)
            closs = loss_func(output, batch_ans)
            bcloss = bcloss_func(ans_mask, batch_ans_mask)
            loss = torch.add(closs, 0.1, bcloss)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if ep % disp_ep == 0:
            ...
            print('Classification Loss: %.4f' % (closs.data[0]))
            print('BCE Loss: %.4f' % (bcloss.data[0]))
