...
###
# Tokenize questions into seperate words, which are stored into 'processed_tokens'.
###
...
###
# Build vocabulary for question and answers, and remove bad words with 'word_count_threshold'.
# Use 'UNK' to replace those bad words in questions.
###
...
###
# Count the occurance of all the answers and store top-'num_ans' answers.
###
...
###
# Encode words within 'max_length' in each sentence into indices.
# label_arrays [N x max_length]: indices of words in each question.
# label_length [N]: length of each question.
# question_id [N]: original question id of each question.
###
...
###
# Encode each answer (words) to index.
###
...
###
# Get unique images in the dataset because one image may correspond to multiple question.
# unique_img: the set of unique images in the dataset
# img_pos: index of each image in 'unique_img'.
###
...
def main(params):
    ...
    # Build a dictionary mapping each answer to its index (1~num_ans).
    # This generates the label for each question.
    atoi = {w:i+1 for i,w in enumerate(top_ans)}
    itoa = {i+1:w for i,w in enumerate(top_ans)}
    # Store the mapping so that we can recover the answer (word) with index.
    ...
    # Store the mapping so that we can map words to indices.
